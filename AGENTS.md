...
- Multi-section ffmpeg export must apply per-section CFR normalization (`fps=fps=TARGET:round=near,trim=duration=D,setpts=PTS-STARTPTS`) to each VFR video segment before concat, clamping each section's video duration to exactly match its audio duration. Without this, VFR sections are slightly shorter than their audio, and the mismatch accumulates across concat segments causing progressive A/V drift. The post-concat fps on `[screen_raw]`/`[camera_raw]` acts as a safety net but does not fix per-section drift.
